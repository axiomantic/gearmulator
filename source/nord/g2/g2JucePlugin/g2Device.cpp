/* The synthLib::Device subclass body.
 *
 * The constructor resolves the firmware state once through ArtifactResolver
 * and never again; the plugin does not look again on its own.
 *
 * getState/setState delegate to g2State.h's serializeState/deserializeState.
 * Every write into _state is an append, never an assign, so the version
 * header the Plugin layer prepended survives. The hand-off pair runs first in
 * both.
 */
...
namespace
{
	 ...
	 * `g2TestConsole/main.cpp`, and each one carries there the measurement
	 * that establishes it. The duplication is deliberate: the composition
	 * has no home in g2Lib -- board.cpp attaches its units and leaves
	 * Region::Sdram, MBAR and the chip-select windows to the composing
	 * party.
	 *
	 * Two of the values are not measurements and are labelled as such at
	 * their site: CS0's and CS4's bases are recorded by no authority. */

	// Measured: the boot loader's `movel #0x10000001,%d0`
	...
	// Measured from the loader's CSAR2 = $1200 and CSMR2 = $007F0001 at
	// loader offsets 0x70 and 0x7c.
...
namespace g2
{
	 ...
	 * with no target on purpose, so the store is the composing party's to
	 * supply. Big-endian, matching the part.
	 *
	 ...
	 * observer after it has completed. The order is load-bearing.
	 *
	 ...
	 * (scheduler.h's CodecRegime) and which beginPlayPhase is the exit
	 ...
	 * own stateSave writes. The plugin state image carries patch and
	 * performance data and no machine snapshot, so a snapshot survives a
	 * getState/boot pair within one session (machineSnapshot(), refreshed
	 * in getState) and does not survive a save to a host project file.
	 * Making it survive means adding an item to the plugin state format in
	 * g2State.{h,cpp}.
	 *
	 * The framework call site is not here: the boot happens during
	 * setStateInformation and prepareToPlay, and prepareToPlay lives in
	 * g2Plugin.cpp. This function is the boot. */
	Device::BootResult Device::boot(const BootRequest& _request)
	{
		...
		// The vector table, big-endian, beside the image. It is a floor and
		// not a fix: the firmware fills the table itself before the first
		// exception is taken, so what is load-bearing here is VBR.
		{
		 ...
		 * firmware writes to UTB into this class's static sink, which feeds
		 * m_midiOutParser. Until this line ran the parser received nothing,
		 * which is why an unbooted Device truthfully reports no MIDI out. */
		...
		// g2::Status. There is no exception and no assertion: the design
		// forbids the first and a release build removes the second, so the
		// rejection is observable through the pair and through nothing else.
		 ...
		 *
		 * It is not one of the six steps and is not notified as one:
		 * inventing a seventh notification would make the order unreadable.
		 *
		 ...
		 * the design describes. The budget is the ceiling
		...
		/* Why the regime rule holds across a restore, and why step 5 needs
		...
		// holding exactly L frames, the chain-health counters zero and the
		// recorded owning-thread identity cleared, so the first runFrames
	...
	{
		// Sequentially consistent, and one of the seq_cst operations of the
		// hand-off pairing. It is NOT an acquire load: the audio thread
		// has touched the object before, so the load is the second half of a
		// store-load pair and only seq_cst orders that pair.
	...
	{
		// The hand-off pair runs first. Insert, never assign, holds through
		// the delegate: serializeState's first write is push_back and every
		// later one is an append, so the header the Plugin layer already put
		// in _state survives.
		beginStateChange();

		/* The machine snapshot is taken here, inside the hand-off window,
		 * and that is the only place it can be taken: stateSave reads the
		 * emulated machine, and reading it while a callback runs would race
	...
	{
		// Same hand-off, then the parse. A refused image (wrong magic,
		// format version, or geometry) changes nothing and reports failure;
		// a firmware-version mismatch loads the machine and withholds the
		// patch data, with the decision's message carried in the result the
		// delegate returns.
		beginStateChange();
	...
	{
		// These are final values, not stubs: the Plugin queries them once, in
		// its constructor's member-initializer list, and ResamplerInOut
		// stores both as const members that nothing re-reads. They must
		// therefore be final the instant this constructor returns -- before
		// the firmware is loaded, before the boot, and whether or not any
		// artifact was found; the no-firmware path presents 2 and 2 as well.
	...
	{
		// See getChannelCountIn: same const-stored pair, same
	...
	{
		// The DSP clock is a gated constant and this design does not offer to
		// vary it: accept 100 and refuse anything
	...
	{
		// The DSP clock is the numerator of the gated constant; timebase.h is
		// its declaration site.
		return G2_DSP_CYCLES_PER_FRAME_NUM;
	}

	/* readMidiOut carries only what the machine originated. The source is
	 * the emulated UART0 transmit register: Uart0 delivers each byte the
	 * firmware writes to UTB to the MidiOutFn callback this class installs on
	 * it, and the callback feeds the byte into m_midiOutParser, whose
	...
	void Device::processAudio(const synthLib::TAudioInputs& _inputs, const synthLib::TAudioOutputs& _outputs, const size_t _samples)
	{
		...
		// to be moved: first it sets m_inCallback, then it reads isValid().
	...
	{
		// Submission, not application: the framework calls
		...
		// future as the buffered audio.
		//
		// One accepted framework limit, recorded and not fixed here: at a
		// host rate other than 96 kHz, all MIDI of
	...
	{
		// The message thread's sequence. The
	...
	{
		// The closing store stays RELEASE: the seq_cst operations are the
		// audio-thread ones and the message-thread ones above; a false store
		// here closes a callback and orders nothing that needs seq_cst.
		m_ready.store(true, std::memory_order_release);
	}
	 ...
	 * and private by design, so the pin lives in a nested class of Device,
	 * where the access exists. The hand-off pairing is written against both
	 * flags being std::atomic<bool>. */
