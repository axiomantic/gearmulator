/* g2Plugin.h -- the synthLib::Plugin subclass.
 *
 * The constructor calls synthLib::Plugin::setResamplerMode with
 * Resampler::Mode::MameHq, so the mode is set here and never inherited. The
 * framework default is Legacy -- the Resampler constructor default
 * (resampler.h:30) and ResamplerInOut::m_mode's initializer
 * (resamplerInOut.h:43) -- and Legacy is about 57 dB short of the required
 * stopband figure and puts the 44.1 kHz passband edge at 19,845 Hz, below the
 * 20 kHz the passband must be flat to. Neither failure has a symptom a
 * listener would attribute to the resampler, which is why the assertion is one
 * line and the defect two.
 *
 ...
 * not run when the body executes. setResamplerMode is therefore a pure member
 * write here; the pre-warm runs later, at the first setHostSamplerate (the
 * framework's prepareToPlay analogue), with the mode already MameHq.
 *
 * The side effect a fixture must plan for. setResamplerMode is a no-op only
 * while the mode already matches; on a change it calls recreate(), and
 * recreate() runs the pre-warm with an empty process callback -- the Device is
 * never invoked during it. A fixture that counts Device callbacks must
 * therefore set the mode before it starts counting, or the one recreate fired
 * by the mode change pollutes the count. This file sets it in the constructor,
 * so for every g2::Plugin instance the pre-warm is over before any caller
 ...
 * records what it set and reports it through resamplerMode().
 */
...
namespace g2
{
	...
	public:
		...
		/* The mode this class set: MameHq, not the framework default
		 * Legacy. */
		synthLib::Resampler::Mode resamplerMode() const noexcept { return m_resamplerMode; }

		/* Both `B` and the reported latency are computed here, from the
		 * same host block and the same host rate, so the two can never
		 * disagree about which prepareToPlay they belong to.
		 *
