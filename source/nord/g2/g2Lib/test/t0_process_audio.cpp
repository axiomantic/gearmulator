/* t0_process_audio.cpp -- the check of PLG-4 step 1 (plan section 17; the
 * gate that was PLG-4's own Check: line, now a conjunct of the merged PLG-4
 * block, §24.6 row W3-390). Design sections 14.7, 13.6 and 13.10 rule 3.
 *
 * WHAT THE CHECK OWNS: the audio callback's choreography.
 *
 *   1. First it sets m_inCallback, THEN it reads isValid(). The
 *      set-before-test order is half of the pairing that makes the state
 *      hand-off safe, so it is not free to be moved. The required-red
 *      mutation moves the set after the isValid read and the order probe
 *      goes red.
 *
 ...
 *
 *   3. On isValid() true: the staged MIDI leaves first (design row 7.31's
 *      "before runFrames for the same block"), then ONE call to
 *      Scheduler::push, then runFrames, then pull, then a read of
 *      Scheduler::faulted(), and finally m_inCallback is cleared. THE CALL
 *      ORDER IS WHAT FIXES BOTH CODEC QUEUE CAPACITIES AT L + B (design
 *      section 13.6): push delivers a whole block before runFrames consumes
 *      any of it and runFrames produces a whole block before pull takes any
 *      of it. The order is asserted through a harness Scheduler mock that
 *      records the call sequence -- the plan's own wording for this test's
 *      method.
 *
 * THE HARNESS SCHEDULER. A real g2::Scheduler cannot record: its methods
 * are not virtual and its constructor is private behind the create factory.
 * The seam the Device itself provides is the borrowed Scheduler pointer the
 * ready branch drives -- the harness hands in a Recorder through the same
 * pointer production's Scheduler occupies. The recorder answers every call
 * the design's order names and records each, so the choreography under test
 * -- and nothing about the Scheduler's internals -- is what the case
 * exercises.
 *
 * THE ORDER PROBE FOR THE REQUIRED-RED: the plan's red is "moving the set
 * after the isValid read". The probe drives the REAL processAudio against
 * the REAL isValid while a probe thread runs the message half's store-load
 * (m_ready false, then read m_inCallback) in a tight hammer, and counts the
 * rounds in which the callback's output was ZEROED while its counter
 * ADVANCED in the same instant-window -- the observable signature of a
 * callback that read isValid() BEFORE setting m_inCallback is that a state
 * change can interleave into the callback's Scheduler section, and the
 * observable that catches it is the recorder's log: a callback whose
 * ready-branch Scheduler calls ran WHILE the message thread's
 * beginStateChange spin was still returning. Under the real device's order
 * that count is zero in every round; under the mutated order it is
...
namespace
{
	 ...
	 * in the order the calls happen. The design's order is push ->
	 * runFrames -> pull -> faulted. */
	...
	/* THE RECORDER. Implements the Device's own four-call Scheduler-driver
	 * contract (ISchedulerDriver, g2Device.h), recording each call's phase
	 * and arguments. The seam is the one the Device itself declares: the
	 * ready branch drives m_driver, and installDriver() replaces what that
	 * pointer names. Scheduler's methods are not virtual and its
	 * constructor is private behind the create factory, so the driver
	 * interface -- which spells the four-call contract the design fixes --
	 * is the seam, and the recorder is what sits behind it in this
	 * fixture. */
...
int main()
{
	 ...
	 * buffers are zeroed, the counter STILL advances (PLG-1 landed that in
	 * the not-ready branch's tail and PLG-6's conversion reads it), and the
	 * Scheduler is touched not at all. */
	{
	 ...
	 * machine, run one callback, and read the recorder's log: exactly four
	 * Scheduler calls, in the design's order, with the right arguments. */
	{
		...
		// THE EGRESS. The recorder answers silence, so the outputs read
		// zero after the callback -- and they were WRITTEN, not preserved:
		// the pre-poisoned 1.0f values are gone.
	...
	/* ------------- Case group 3. THE FAULT RESPONSE. faulted() answers
	 * true after runFrames; the one response of design section 13.10.3 step
	 * 4 is a release store of false into m_ready, observed as isValid()
	 * going false and the NEXT callback taking the silence path. */
	{
	...
	/* ------------- Case group 4. THE SET-BEFORE-TEST ORDER PROBE, and the
	 * required-red it observes. The pairing of design section 13.10 rule 3
	 * is a store-load pair on both sides: the audio thread stores
