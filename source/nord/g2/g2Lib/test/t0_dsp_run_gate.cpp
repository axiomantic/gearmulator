...
namespace
{
	...
	void driveClosed(const bool* const programLanded, const char* const what)
	{
		...
		checkEqual(ctx.acc, oneAllocation,
			...
			"assertion does not answer it. The alloc() call at the top of "
			"dspJob() stands AHEAD of the run gate, so a shut gate advances "
			"the accumulator by one whole allocation; that is what "
