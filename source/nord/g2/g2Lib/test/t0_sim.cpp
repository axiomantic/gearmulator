...
int main()
{
	...
	// each carry a bound check for that reason.
	//
	// The access below arrives through the same decode every other group in
	// this file uses. MemoryMap::decode examines only the START address, so
	// MBAR+0x3fe is inside the one-kilobyte window and a 32-bit access there is
	// handed to the SIM as offset 0x3fe. The SIM then walks four bytes, 0x3fe to
	// 0x401, and the last two lie past the top of the window.
	//
	// Group 16 pins that a wide access does not stop at a REGISTER boundary.
	// This group pins that it does stop at the WINDOW boundary. Only one of the
	// two bounds the storage.
	//
	// Without the write bound the model stores two bytes of a firmware write
	// into m_writeProtect, which unprotects reserved MBAR space that UM Table
	// 9-5 footnote 1 requires to stay inert. Without the read bound the model
	// hands protection bytes back as if they were register contents.
	//
	// No register covers MBAR+0x3fe, so Sim::find answers null and the access is
	// traced as any other unmodelled access is. The offset asserted is the one
	...
	{
		// The write bound, observed exactly where an unbounded write would land.
		// The first byte past m_space is the write-protect byte of MBAR+0x000, so
		// an unbounded write at 0x3fe unprotects part of a reserved offset.
		// modelWritableMask recovers that protection from behaviour.
		//
		// The first case is the before picture: it establishes that the mask this
		// group watches starts fully protected, so the second case's 0x00 is a value
		// that was observed to be reachable.
		Bus bus;
	...
	//
	// The model keeps the byte-run reading: the bytes that land inside a
	// register do reach it, and the log line records the offset the access
	// started at. The write protection is per byte, so it is the mask of the
	// bytes themselves and not any decision taken at the access's start offset
	// that keeps a reserved hole inert; and sim.h defines the log as carrying
	// one line for every access to an offset the manual assigns to no register
	// this model carries - the offset it was given, singular. Logging by start
	// offset and transferring by byte run are orthogonal, which is why the two
	// halves below assert them separately.
	//
	...
	// could move; this hole cannot.
	//
	// A 32-bit write at $106 that reached DACR0 while the log called the access
	// unmodelled would be a misdirected write that reads back exactly like a
	// modelled one.
	{
		Bus bus;
		mcf5307_bus_status status = MCF5307_BUS_OK;
