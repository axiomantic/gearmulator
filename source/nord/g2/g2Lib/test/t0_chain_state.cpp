 ...
 * wrapper and builds each transmit frame from scratch, so replaying the same
 * quanta against a restored adapter must reproduce the first replay exactly.
...
namespace
{
	 ...
	 * and the second bus / ESAI_1 on MemArea_Y). The transmit wrappers read the
	 * emulated ESAI's own M_TUE bit, so the flag rule needs a real peripheral
	 * behind it. */
	struct PositionEsai
	{
	...
	constexpr unsigned kQuanta    = 100u;
	...
	/* One quantum of the four-phase procedure: swap, ingress, run, egress. The
	 * run phase here is synthetic -- every position's transmit wrapper is
	 * fired directly -- because this file is about what survives a save and a
	 * load and not about who calls the wrappers.
	 *
	 * Each counter condition is driven by a different mechanism:
	 *   - the audio M_TUE bit is set on a 5-quantum cadence, which leaves an
	 *     audio written flag clear and raises underrunFrames;
	 *   - the second-bus M_TUE bit is set on an 11-quantum cadence, which
		...
		const uint64_t _from, const unsigned _count)
	{
		...
		for(uint64_t f = _from; f < _from + _count; ++f)
		{
			...
			for(unsigned p = 0; p < kPositions; ++p)
			{
				_esai[p].audioEsai.writestatusRegister(((f + p) % 5u == 0u) ? kMtu : 0u);
				...
				if(window || forced)
				{
					_esai[p].secondEsai.writestatusRegister(((f + p) % 11u == 0u) ? kMtu : 0u);
					const dsp56k::Audio::TxFrame secondTx = makeTxFrame(1u, f, p, 2u);
					_adapter.secondTxCallback(p)(frameIndex, secondTx);
				}
			}

			/* Read and discarded: the egress content is asserted through the
			 * digest, and the call is here so the quantum is the whole
			 * four-phase procedure and not three quarters of it. */
			g2::Frame out{};
			_adapter.extractCodecSink(out);
		}
	}
...
int main()
{
	...
	/* A zero-byte image would satisfy every equality below, because an empty
	 * snapshot restores nothing and changes nothing. The depth comparison is
	 * what makes the size structural rather than a constant: an image that did
	 * not carry the rings cannot grow when the rings get deeper. */
	{
	 ...
	 * "identical" below is satisfied by an adapter that never changes. */
	{
		char where[256];
	...
	/* The direct round trip: it runs no quanta at all, so nothing but
	 * stateSave and stateLoad can make it pass. */
	...
	/* The behavioural half: the round trip above says the bytes came back, and
	 * this says the machine they describe runs the same way afterwards. */
	runQuanta(adapter, esai, kQuanta, kQuanta);
