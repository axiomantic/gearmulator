...
namespace
{
	...
	public:
		...
		// One counter per 16-bit word of SDRAM. A histogram rather than a probe
		// list, so the file can ask the run which address it read most instead of
		// only asking about addresses it names itself.
		struct Hottest
		{
			uint32_t absolute = 0;
			uint64_t hits     = 0;
...
int main()
{
	...
	g2::test::runGated(resolver, std::cout, counters, [&]() -> bool
	{
		...
		for(const RunResult* const arm : { &control, &patched })
		{
			...
			/* THE INVARIANT NOW READS `completed` AND NOT `accepted`, AND THAT
			 * IS A UNIT REPAIR RATHER THAN A WEAKENING. `accepted` counts
			 ...
			 * this equality needs. board.h states the same thing at the
			 * declaration. */
			check(u.drained == u.completed + (u.held ? 1u : 0u),
				std::string("NOTHING DRAINED IS LOST: ") + std::to_string(u.drained)
				+ " frames left the hub, " + std::to_string(u.completed)
				+ " crossed to the device whole and " + (u.held ? "1 is" : "0 are")
