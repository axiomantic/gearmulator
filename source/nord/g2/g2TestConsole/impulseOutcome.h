...
namespace g2console
{
	...
	struct ImpulseObservation
	{
		...
		/* The arrival instrument's own known positive, a different control from
		 * observerSelfTest. That one drives the detector's two predicates over
		 * two frames the program built; it holds even when nothing between the
		 * tail DSP and `Scheduler::pull` works, because no part of that path is
		 * on its evidence. These two fields carry the result of a walk in which
		 * a known sentinel was placed at the tail position's transmit source
		 * and then read back out of the sink, so they are a statement about the
		 * arrival path itself.
		 *
		 ...
		 * known value has not earned the right to be believed about an unknown
		 * one, so both are required. */
	...
	constexpr ImpulseOutcome classify(const ImpulseObservation& _o)
	{
		...
		/* The comparator self-test proves the detector; the frame count proves
		 * the sink delivered something. Neither proves the path between the
		 * tail DSP and that delivery can carry a value: a sink that hands over
		 * a zero frame every quantum satisfies both and reports `arrival=-1`
		 * forever. The sink control is the known positive for exactly that
		 * path, and it must have arrived and arrived unchanged. */
